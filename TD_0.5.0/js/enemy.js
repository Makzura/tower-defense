...
function Enemy(path, health, typeId, overrides) {
  ...
  // so a Veil Dart halfway home reads 0.5 x path.length exactly as a walker
...
Enemy.TYPES = {
  ...
  // has made it.
  //
  // IT HAS ITS BODY NOW (2026-08-30). It shipped for a few hours as "Skimmer"
  // drawn as the fallback sphere, which is what the board gives any type it has
  // no mesh for. The Veil_Dart pack then arrived carrying the same creature to
  // the letter -- 50 HP, camo, flying, ignores the road, never attacks -- and
  // an approved concept sheet with a name on it, so the type took the art's
  // name rather than the placeholder's. An enemy id is not a persistence format
  // (only tower ids are saved), so the rename costs nobody a save.
  //
  // `js/gl/models/enemy-veil_dart.js` is the import, generated by the
  // repository's own `tools/glb_to_animated.py` from `../glb/veil-dart.glb`.
  // 235 triangles and four named clips; `travel` is the one it flies on -- see
  // ENEMY_CLIP in js/gl/gl-world.js.
  veil_dart: {
    id: "veil_dart",
    displayName: "Veil Dart",
    health: 50,
...
Enemy.prototype.positionAt = function (progress) {
  ...
  // chord's own normal, so a flight of Veil Darts arrives spread out rather than
...
Enemy.prototype.currentSpeedUlps = function () {
  ...
  // that basin quickly" -- and a Veil Dart is over the basin, not in it. Slows,
...
Enemy.prototype.update = function (dt) {
  ...
  // reads the same as it always did for every body but a Veil Dart.

...
def write_js(model, filename):
    ...
    # The chassis stamp goes in the COMMENT as well as the object: a human
    # scanning a diff reads the header, and a tool reads the field. `grep
    # CHASSIS_VERSION js/gl/models/enemy-*.js` is then a one-command answer to
    # "which of these were built from the same chassis", which is the check the
    # chassis module's own header now tells people to run.
    stamp = []
    if model.get("chassisVersion") is not None:
        stamp.append("// Built from tools/blender/enemy_chassis.py, "
                     "CHASSIS_VERSION %d." % model["chassisVersion"])
    ...
    ] if model.get("chassisVersion") is not None else []) + [
...
def _chassis_version_of(module):
    """The CHASSIS_VERSION a module was built against, or None if it is not
    chassis-built.

    Looks for the `enemy_chassis` MODULE OBJECT in the build script's globals
    rather than for an attribute called `chassis`, so a body that imports it
    under any name is still stamped and a body that does not import it at all
    is silently skipped. That is what keeps this self-maintaining as batch 2
    adds bodies -- see the `grep -l "import enemy_chassis"` note in that file's
    header, which is the same question asked of the source instead of the scene.
    """
    for value in vars(module).values():
        if getattr(value, "__name__", None) == "enemy_chassis":
            return getattr(value, "CHASSIS_VERSION", None)
    return None
...
def main():
    ...
    for name, filename, build in targets:
        # Cleared per target, so one model's bands can never leak onto the next
        # in a multi-target run -- which is every run of `--only=enemy-`.
        _BUILD_INFO.clear()
        frames = build() or 0
        ...
        model["bands"] = _BUILD_INFO.get("bands")
        model["chassisVersion"] = _BUILD_INFO.get("chassisVersion")

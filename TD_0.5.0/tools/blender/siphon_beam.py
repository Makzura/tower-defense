...
# --- the socle's fixed points, READ rather than retyped ----------------------
#
# THESE USED TO BE TWO LITERALS. `HANDS = (0.055, 0.305, 1.045)` and
# `RING = (0.315, 0.395, 1.190)`, copied out of SIPHON-SOCLE.md and sitting a
# few hundred lines from the identical pair in siphon_idol.py. Two generators
# holding the same constant by hand is a drift waiting to happen, and it is the
# worst kind: both files build cleanly, both look right in isolation, and the
# beam starts a few pixels off a ring that has moved.
#
# siphon_idol.py MEASURES its origins off the built geometry and writes
# siphon_origins.json. That file is therefore the only place the number is true,
# and this file reads it. Nothing here re-derives it and nothing here defaults.
#
# IT IS A HARD ERROR, NOT A FALLBACK. A default would mean silently building
# eight beams against a stale origin -- precisely the failure this change exists
# to make impossible. siphon_origins.json is committed, so the only way it can
# be missing is a half-checked-out tree, and that deserves to stop the build.
...
def _read_origins():
    if not os.path.isfile(ORIGINS_PATH):
        raise SystemExit(
            "siphon_beam.py reads the beam origins out of\n    %s\nand that "
            "file is not there. It is GENERATED by tools/blender/"
            "siphon_idol.py -- run that first. The origins used to be retyped "
            "in this file; they are read now so the two generators cannot "
            "drift apart." % ORIGINS_PATH)
    with open(ORIGINS_PATH, "r", encoding="utf-8") as handle:
        got = json.load(handle)
    socle = got.get("socle") or {}
    for key in ("HANDS", "RING"):
        p = socle.get(key)
        if not (isinstance(p, list) and len(p) == 3):
            raise SystemExit(
                "%s has no socle.%s -- siphon_beam.py cannot place a beam "
                "without it. Re-run tools/blender/siphon_idol.py."
                % (ORIGINS_PATH, key))
    return got


ORIGINS = _read_origins()
HANDS = tuple(ORIGINS["socle"]["HANDS"])
RING = tuple(ORIGINS["socle"]["RING"])
...
SPEC_STATES = {
    ...
    "seeking": {
        ...
        "note": "detached: reaches only 0.55 of the run and ends in 3 groping "
                "filaments of unequal length. THE SAME CORD AS `thread`, "
                "THINNER AND SEARCHING -- see `retune`.",
        "retune": "Owner: 'the passive rays who dangle around when he is not "
                  "attacking doesn't fit the ray he uses when attacking, which "
                  "is confusing.' The mismatch was GEOMETRY, not colour: "
                  "seeking and thread already shared body/bead/rim roles and "
                  "differed only in `core`. Against thread, seeking was 2.1x "
                  "thinner at the far end, sagged 10x (0.300 vs 0.030), swayed "
                  "7.3x (0.160 vs 0.022), carried kinks thread has none of, "
                  "twisted 1.8x with a waver term, scrolled at 55% of thread's "
                  "speed, and bunched 9 knots into the tower-side two thirds "
                  "while thread spreads 14 proud ones along the whole run. "
                  "Every one of those is now moved most of the way to thread's "
                  "value, and `skin`/`skin_dark` are added so ROLES can give it "
                  "thread's core. It stays THINNER and keeps the droop and the "
                  "sweep, because idle must still read as weaker and searching "
                  "-- it is the same rope, slack, not a different one."},
...
# --- the per-frame origin table ----------------------------------------------
#
# THE RING MOVES NOW. Until the sceptre became one rigid body carried by the
# hand, A3+'s beam origin was a constant: the rings were kept out of the
# animated group precisely so the point could not move. Now the whole sceptre
# swings, so WHERE THE BEAM LEAVES FROM depends on which frame of the body is on
# screen, and a single point cannot say it.
#
# So siphon_idol.py measures the origin on every frame off the posed vertices
# and writes the sequence into siphon_origins.json; this carries it through to
# the renderer unchanged. Nothing here computes an origin -- it only re-keys and
# re-checks what the idol generator measured.
#
# THE KEYS ARE LOWERCASE AND THAT IS NOT A STYLE CHOICE. js/gl/gl-world.js:377
# `siphonGroup()` returns "base", "a1".."a5", "b1".."b5", and
# js/gl/siphon-beam-draw.js:230 `bodyKey()` mirrors it line for line; that is
# what indexes this table. The spec's own `originByTier`, which sits directly
# above this in the output, is UPPERCASE and is left alone -- it answers a
# different question (hands or ring, per tier) and re-casing it is a separate
# change. Copy the neighbour's convention here and every lookup returns
# undefined, the renderer falls back to the static ring, and the whole change
# ships as a silent no-op that looks perfect in a screenshot.
#
# THERE ARE DELIBERATELY NO b1..b5 ROWS. The b bodies are authored by
# tools/blender/siphon_abyss.py, which never writes the origins file, and a
# path-B Siphon pours from the HANDS -- there is no ring on it to follow, and
# siphon_abyss.py gates its own origin as unable to move (its brackets are in
# the STATIC group and no moving vertex may come within 0.035 of HANDS). A
# b-tier lookup miss is therefore a CONTRACT, not a hole, and the consumer
# already treats it as one: siphon-beam-draw.js:274-275 returns early and
# silently for any key beginning "b".
#
# Filling them in anyway would be worse than leaving them out, and measurably
# so. siphon_abyss.py ships 4 frames per b body against siphon_idol.py's 25, so
# a 25-long b row trips `modelFrames !== row.length` (siphon-beam-draw.js:334)
# and a 4-long one trips `declared !== row.length` (line 333). Either way every
# B-path Siphon on the board earns a console warning to describe a point that
# never moves.

def _origin_frames():
    """(table, frame count) -- the per-frame origin, one row per BODY.

    Returns ({}, 0) when siphon_origins.json carries no `frames` arrays, which
    is the state of the file before the idol generator's per-frame export lands.
    `spec()` then omits both fields entirely rather than emitting a degenerate
    one-entry table, because the renderer's no-table path is the old static
    behaviour -- correct, silent, and exactly what we want in the window where
    the two generators are out of step."""
    rows, count = {}, 0
    for tier, entry in (ORIGINS.get("origins") or {}).items():
        seq = entry.get("frames")
        if not seq:
            continue
        key = tier.lower()
        point = [round(float(v), 10) for v in entry.get("point", [])]
        first = [round(float(v), 10) for v in seq[0]]
        # FRAME 0 IS THE REST POSE, and `point` is the rest pose. gl-world shows
        # frame 0 to an idle Siphon, so if these two ever disagree the beam
        # jumps the instant the tower acquires a lock -- and it jumps back when
        # it loses one. Equality is asserted rather than assumed because the two
        # values travel through different code paths in the generator.
        if point != first:
            raise SystemExit(
                "siphon_origins.json: origins.%s has point %s but frames[0] "
                "%s. Frame 0 IS the rest pose -- gl-world draws it for an idle "
                "Siphon -- so those must be the same point. Fix "
                "tools/blender/siphon_idol.py, not this file."
                % (tier, point, first))
        if count and len(seq) != count:
            raise SystemExit(
                "siphon_origins.json: origins.%s has %d frames but an earlier "
                "tier has %d. `originFrameCount` is one number for the whole "
                "spec and js/gl/siphon-beam-draw.js:333 warns when a row "
                "disagrees with it, so the tiers have to march together."
                % (tier, len(seq), count))
        count = len(seq)
        rows[key] = [[float(v) for v in p] for p in seq]
    return rows, count
...
def spec():
    ...
    origin_rows, origin_count = _origin_frames()
    out = {
        ...
        "originFrameCount": origin_count,
        "originFrames": origin_rows,
    ...
    for name, _fn, path, origin in STATES:
        ...
        if name in MEASURED:
            ...
            d["beads"] = beads
        out["states"][name] = d
    # Both fields are declared in the literal above so they land next to
    # `originByTier` where a reader expects them, and dropped as a pair when
    # there is nothing to say. `del` keeps the order of everything else, and an
    # ABSENT table is the renderer's documented static path
    # (siphon-beam-draw.js:321 `if (!table) return stat`) -- an empty one is not.
    if not origin_rows:
        del out["originFrameCount"]
        del out["originFrames"]
    return out


def write_spec(path):
    got = spec()
    body = json.dumps(got, indent=1, sort_keys=False)
    rows = got.get("originFrames") or {}
    if rows:
        state = ("// Rows present: [%s], %d frames each, which must equal the "
                 "body's" % (", ".join(sorted(rows)), got["originFrameCount"]))
        state2 = "// own frames.length or siphon-beam-draw.js:334 will say so."
    else:
        state = ("// NO originFrames IN THIS BUILD: siphon_origins.json "
                 "carries no `frames`")
        state2 = ("// arrays yet, so the renderer takes its static-origin "
                  "path unchanged.")
    lines = [
        ...
        "//",
        "// `originFrames` is keyed by gl-world.js:377 siphonGroup()'s"
        " LOWERCASE",
